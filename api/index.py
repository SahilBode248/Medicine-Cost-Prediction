"""
Unified Vercel Python entrypoint: /api/index.py
Dispatches:
  - GET  /api/metadata -> metadata handler
  - POST /api/predict  -> predict handler
  - GET  /api/predict  -> info handler
"""
...
# Load trained pipeline once on startup
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.endswith("/metadata"):
            ...
            self.wfile.write(json.dumps(_metadata).encode("utf-8"))
            ...
            self.wfile.write(json.dumps({
                "status": "ok",
                "endpoints": ["/api/metadata", "/api/predict"]
            }).encode("utf-8"))
